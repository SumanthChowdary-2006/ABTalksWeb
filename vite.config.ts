...
export default defineConfig(({ mode }) => {
  // passes `--mode development` for cached-preview builds.
...
function appSiteConfiguration(config: AppSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "ABTalks"
...
/**
 * Serves a blank render-target page at /.make/kit.html.
 * The page exposes a registry of every file matching `storiesGlob` on
 * window.__APP__.stories.
